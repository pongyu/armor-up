...
final class DeclineDefense extends GameAction {
  /// True when this decline was generated by the net layer's
  /// defense-response deadline expiring, rather than the player
  /// themselves choosing to decline. Purely informational: it does not
  /// change how the decline resolves (same Fellowship-fallthrough /
  /// attack-landing logic either way), only which [GameEvent] is logged,
  /// so the UI can show "Player N ran out of time" instead of implying a
  /// deliberate choice. The engine itself never sets this - it only ever
  /// arrives as caller-supplied input, from [HostServer]'s timeout timer.
  final bool isSystemDecline;

  const DeclineDefense({
    required super.playerId,
    this.isSystemDecline = false,
  });

  @override
  String toString() => 'DeclineDefense($playerId'
      '${isSystemDecline ? ', system' : ''})';
}
